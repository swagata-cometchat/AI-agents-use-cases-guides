...
> Create a Mastra agent that orchestrates employee onboarding: collects info, processes I-9 docs, verifies identity & work authorization, runs background checks, provisions access, checks compliance — then connect it to CometChat.
...
- REST endpoints for each onboarding step: initiate → personal info → employment info → documents → identity → background → system access → tax → compliance → status.
...
- **Key Tools**: ./src/mastra/tools/verification-tools.ts
- **Agents**: ./src/mastra/agents/onboarding-agent.ts
- **Routes (handlers)**: ./src/mastra/server/routes/onboarding.ts
...
(If you expose Swagger UI, adjust link to: `http://localhost:4001/swagger-ui`)

***

## How it works

This example implements a workflow‑oriented Employee Onboarding Agent that:
...
- Exposes two chat agents:
  - `employee-onboarding` — conversational guidance & next-step explanations.
  - `hr-workflow-manager` — decision / escalation logic & routing recommendations.
- Sanitizes errors via `safeErrorMessage.ts` before returning responses.

Key components:
- Agents: `src/mastra/agents/onboarding-agent.ts`
- Tools: `src/mastra/tools/verification-tools.ts`
- Handlers: `src/mastra/server/routes/onboarding.ts`
- Route registry: `src/mastra/server/routes.ts`
- Server bootstrap: `src/mastra/index.ts`
...
<Steps>
  ...
  <Step title="Walk the workflow">
    Call endpoints in order: initiate → employee-info → employment-info → process-document(s) → verify-identity → background-check → create-system-access → tax-info → check-compliance → status.
  </Step>
  <Step title="Connect to CometChat">
    In Dashboard → AI Agents, set Provider=Mastra, Agent ID=<code>employee-onboarding</code>, Deployment URL=public generate endpoint.
  </Step>
</Steps>
...
- Ensure agent generation endpoints (e.g., `/api/agents/employee-onboarding/generate`) are accessible (Mastra auto‑generates if configured accordingly—update index if needed to import your `apiRoutes`).
- Provide CORS / logging / persistence (add DB later — current demo uses in‑memory `Map`).
...
<Steps>
  ...
  <Step title="Add AI Agent">Navigate to <b>AI Agents</b> → Add New.</Step>
